...
def submit():
    ...
    try:
        ...
        disassociate = O[~mask]
        disassociate[:,3]=-1.1808
        np.random.shuffle(disassociate)
        # equilibrium charge
        total = 0
        supplementalO = []
        for i in range(len(disassociate)):
            total = total - disassociate[i,3] 
            supplementalO = np.append(supplementalO,disassociate[i],axis = 0)

            if total >= need:
                break
        supplementalO = supplementalO.reshape(len(supplementalO)//4,4)
        idx = np.random.choice(supplementalO.shape[0], len(supplementalO)-1, replace=False)
        supplementalO = supplementalO[idx,:]
